...
use winapi::um::d3dcompiler::{D3DCOMPILE_ENABLE_STRICTNESS, D3D_COMPILE_STANDARD_FILE_INCLUDE, D3DCompile, D3DCOMPILE_DEBUG};
    ...
    pub fn new(renderer: &Renderer, layout: BufferLayout, vertex_src: &str) -> VertexShader {
        unsafe {
            ...
            let result = D3DCompile(
                data, vertex_src.len(), null_mut(), null_mut(),
                D3D_COMPILE_STANDARD_FILE_INCLUDE, win32_string_short("VSMain").as_ptr() as *const i8, 
                win32_string_short("vs_5_0").as_ptr() as *const i8, D3DCOMPILE_DEBUG | D3DCOMPILE_ENABLE_STRICTNESS, 0, &mut vs_buffer_ptr, &mut error_buffer_ptr);
            ...
            let mut first = true;
            let mut names = vec![];
            let input_desc: Vec<D3D11_INPUT_ELEMENT_DESC> = layout.elements.iter().map(|element|  {
            ...
            if FAILED(result) {
                panic!("Failed to create input layout {:?} {:x}", vertex_src.chars(), result);
            }
            return VertexShader {
...
impl FragmentShader {
    pub fn new(renderer: &Renderer, fragment_src: &str) -> FragmentShader {
        unsafe {
            ...
            let data: *const c_void = mem::transmute(short_string.as_ptr());
            let result = D3DCompile(data, fragment_src.len(), null_mut(), null_mut(),
                                            D3D_COMPILE_STANDARD_FILE_INCLUDE, win32_string_short("PSMain").as_ptr() as *const i8, win32_string_short("ps_5_0").as_ptr() as *const i8, D3DCOMPILE_DEBUG | D3DCOMPILE_ENABLE_STRICTNESS, 0, &mut fs_buffer_ptr, &mut error_buffer_ptr);
            let error_buffer: Box<ID3DBlob> = Box::from_raw(error_buffer_ptr);
            if FAILED(result) {
                let len = error_buffer.GetBufferSize();
                let chars = error_buffer.GetBufferPointer() as *const u8;
...
impl Shader {
    pub fn new(renderer: &Renderer, buffer_layout: BufferLayout, vertex_src: &str, fragment_src: &str) -> Shader {
        return Shader {
