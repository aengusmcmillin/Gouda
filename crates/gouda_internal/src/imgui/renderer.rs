...
impl GoudaImguiRenderer {
    ...
    pub fn render(&self, scene: &Scene, draw_data: &DrawData) {
        for draw_list in draw_data.draw_lists() {
            ...
            let translate =
                Matrix4::from_nonuniform_scale(2.0 / (right - left), 2.0 / (top - bottom), -1.);
            let scale = Matrix4::from_translation(Vector3::new(
            ...
            for cmd in draw_list.commands() {
                match cmd {
                    DrawCmd::Elements {
                        ...
                        scene.submit_imgui(
                            &vbuf, ibuf, count, vtx_offset, idx_offset, texture, matrix,
                        );
                    }
    ...
    fn upload_font_texture(renderer: &Renderer, fonts: &mut imgui::FontAtlas) -> RenderableTexture {
        ...
        let image = Image {
            ...
            data: Image::data_from_raw_pixels(
